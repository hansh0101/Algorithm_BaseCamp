...
int find_min() {
    // 배열에 들어가는 정수 Z의 범위는 0<=Z<=10,000이므로, 초기 min값을 0으로 할당한다. (0보다 작은 값이 들어올 수 없으니까)
    int min = 0;
    // 배열을 순회하면서, 현재 배열의 값이 min보다 작으면 min값을 최신화해준다.
    for (int i = 0; i < n; i++) {
        if (arr[i] < min) {
            min = arr[i];
        }
...
//    또, 배열을 지역변수로 선언할 시, 각 함수들은 main 바깥에 있기 때문에 함수로 배열을 전달하던지, 함수를 main문 안으로 가져와야 한다.
//    난 귀찮으니까 그냥 전역변수로 쓴다 ... 이게 편하다.
//    for (int i = 0; i < 10000; i++) {
//        arr[i] = 0;
//    }

    cin >> n >> t; // 배열의 크기와 테스트 케이스 수를 입력받는다.
